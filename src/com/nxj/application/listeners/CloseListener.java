 ...
 * providing a WindowListener with some common features that might be used when
 * closing an application. That is you can: <ul> <li>display an application
 * closing confirmation message which will give the user a chance to cancel the
 * close request <li>invoke a provide Action. This action might be used to save
 * the state of the application for the next time it is used. </ul>
 *
 * This class will invoke the setDefaultCloseOperation(...) method with an
 * appropriate value. The default will be set to EXIT_ON_CLOSE. The default can
 * be changed by invoking the setDisoseOnClose() method with "true", in which
 * case DISPOSE_ON_CLOSE will be used. This value will be set to
 * DO_NOTHING_ON_CLOSE if the user does not confirm the application closing
 * message. Finally, the close action is free to set this property to any value.
 */
public class CloseListener extends WindowAdapter {
    ...
    /**
     * Display a confirmation message upon closing the application and and
     * invoke a closing action.
     *
     * @param message the confirmation message
     * @param title the text used for the title bar of the dialog
     * @param closeAction the Action to be invoked upon closing the application
